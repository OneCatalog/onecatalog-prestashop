...
### Реализовано на `dev` — полировка: хуки §8 + журнал импорта (версия 0.7.0)
- **Хуки §8**: `actionOnecatalogProductImported` (после импорта товара) и
  `actionOnecatalogPriceStockUpdated` (после записи цены/остатка, сырые офферы) —
  регистрируются при установке, шлются из `Importer`/`B2bSync`. Документация —
  `docs/EVENTS.md`.
- **Журнал импорта**: страница `AdminOnecatalogLog` (последние 200 результатов из
  `onecatalog_log`) + кнопка очистки + вкладка меню.
- **README** расширен (возможности, установка, использование, структура, тесты).
- ✅ **Паритет с OpenCart-портом достигнут** (импорт каталога §1–§12 + B2B §13 + UX +
  справочные сущности + хуки + журнал). Полное соответствие стандарту v1.2.
